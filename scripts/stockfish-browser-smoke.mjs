...
try {
  ...
  if (await more.count()) {
    try {
      ...
      // Top-level nav (no More menu) exposes settings directly.
    }
  }
  // Settings moved into the signed-in account menu; the static smoke server has
  // no auth session, so verify the live Settings version element only when the
  // nav entry exists and skip the click-through otherwise (unit tests plus the
  // sync test already pin the provider/manifest wiring).
