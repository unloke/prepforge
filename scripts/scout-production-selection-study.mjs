...
import { preparationValue, selectPreparationRoutes, nestedRoutes, routeFamily } from '../web-src/scout-preparation-value.js';
...
export const scenarios = [
  ...
  { name: 'families-compete', budget: 2, expected: ['a1','b'], rows: [make('a1',trunk+' f1c4 f8c5',30,100,100),make('a2',trunk+' f1b5 a7a6',28,100,100),make('b','d2d4 d7d5 c2c4 e7e6',22,100,100)] },
  { name: 'family-flood', budget: 3, expected: ['a0','b','c'], rows: [...Array.from({length:8},(_,i)=>make('a'+i,trunk+' choice'+i+' reply',10,100,100)),make('b','d2d4 d7d5',9,100,100),make('c','c2c4 e7e5',8,100,100)] },
];
...
export function study() {
  return scenarios.map(s=>({scenario:s.name, expected:s.expected,
    ...
    coverage:selectPreparationRoutes(s.rows,{limit:s.budget}).map(r=>r.id),
  }));
}
