...
class BrilliantConfig:
    ...
    ``min_reveal_score`` carries the load against false positives. A probe over
    labeled reference moves (``scripts/brilliant_probe.py``) showed real
    sacrifices reveal ≥ 0.365 while a chaotic-pawn-race dud — where ``policy``
    mass is merely spread thin across several roughly-equal rook moves — reveals
    only ~0.20. A 0.30 cut sits in that wide gap. (A first-glance "looks already
    won" cap was tried and removed: its window was razor-thin and it wrongly
    rejected the Immortal queen sac 11.Qxh7+, whose glance is ~0.61.)
