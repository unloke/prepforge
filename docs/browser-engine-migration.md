...
- [ ] **Vite + `web-src/` restructure** — **BLOCKED: no usable Node/npm toolchain on PATH** (a sandboxed `node.exe` resolves but errors `Access is denied`; `npm` not found). Install Node.js LTS and verify `node --version` / `npm --version` before Vite work. Needed anyway for Phase 1's npm packages.
- [ ] **`EngineProvider` / `ServerEngineProvider` seam** — deferred to land *inside* the Vite/`web-src` module layout so the 4700-line `app.js` is refactored once, not twice.
