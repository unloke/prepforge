  ...
  it("does NOT skip the node; preserves the manual move and adds a branch off the mainline", async () => {
    ...
    const engine = {
      candidates: async (fen, count) => {
        ...
        return [cand("e2e4", 1), cand("d2d4", 2)];
      },
    };
...
describe("opponent turn — Maia threshold selection", () => {
...
describe("detail mode — branch recursion", () => {
  ...
  const engine = { candidates: async () => [cand("g1f3")] };
  ...
  it("simple: creates branches but does not recurse into them", async () => {
    ...
    // mainline e7e5 recurses (one Stockfish child); branch c7c5 does not.
    const sfAdds = plannedAdds(plan).filter((c) => c.source === SOURCE.GENERATED_STOCKFISH);
    expect(sfAdds.length).toBe(1);
  });
  ...
  it("balanced: recurses into branches off the mainline path", async () => {
    ...
    const sfAdds = plannedAdds(plan).filter((c) => c.source === SOURCE.GENERATED_STOCKFISH);
    expect(sfAdds.length).toBe(2); // under both e7e5 and c7c5
  });
});
...
describe("ownColor independent of repertoireColor", () => {
  it("recursion follows ownColor; persisted flags are not baked into the plan", async () => {
    ...
    const adds = plannedAdds(plan);
    // root (white to move) is the OPPONENT under ownColor=black → Maia
    expect(adds[0].source).toBe(SOURCE.GENERATED_MAIA3);
    // next ply (black to move) is the USER → Stockfish
    expect(adds.some((c) => c.source === SOURCE.GENERATED_STOCKFISH)).toBe(true);
    assertNoFinalFlags(plan);
  });
});
